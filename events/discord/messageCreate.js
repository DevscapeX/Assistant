...
module.exports = async (client, message) => {
	...
	if (currentGuildSettings.antilink) {
		if (currentGuildSettings.antilink.enabled) {
			...
			if (LINK_DETECTED && (!message.member.roles.cache.has(currentGuildSettings.antilink.rolesExcluded) && !message.member.permissions.has("ADMINISTRATOR"))) {
				if (currentGuildSettings.antilink.allowgif) {
					...
					if (GIF_DETECTED) {
						...
						if (client.spams.get(message.author.id) === (client.guildsettings[message.guild.id].links_spam_time - 3)) {
							message.channel.send(`${message.author}, Next link you will get timed out`)
						}
